...
####Figure4C
#dn/ds and signature composition of SARS-CoV-2
signature_df <- read_csv("../data/sars_signature_prop.csv")
...
scale_factor <- prop_range / dnds_range
dnds_signature_df <- signature_df %>% dplyr::left_join(dnds_df, by=c("months"="start"))

plot_syn <- ggplot(signature_df %>% filter(sub_type == "syn"), aes(x = months)) +
            geom_col(aes(y = prop, fill = signature), alpha=0.5,  position = "stack") +
            geom_point(
              data = dnds_df,
              aes(x = start, y = (dnds - dnds_min) * scale_factor + min(signature_df$prop)),
              color = "black", size = 0.5) +
            geom_line(data = dnds_df,
                      aes(x = start, y = (dnds - dnds_min) * scale_factor + min(signature_df$prop)),
                      color = "black", size = 0.1)+
            scale_fill_manual(
              values = signatrue_color,
              labels = c("SARS-CoV-2\nsignatureA", "SARS-CoV-2\nsignatureB", "SARS-CoV-2\nsignatureC")) +
            scale_y_continuous(
              name = "Signature compositions",
              sec.axis = sec_axis(
                ~ (. - min(signature_df$prop)) / scale_factor + dnds_min,
                name = "dN/dS")) +
            labs(x = "Month (since 2019/12)", fill = "Mutation signature", title="Synonymous spectra") +
            my_theme() +
            theme(
              axis.title.y.right = element_text(angle = 90,face="italic"), # 右侧 y 轴标签方向
              legend.position = "none")

plot_nonsyn <- ggplot(signature_df %>% filter(sub_type == "non_syn"), aes(x = months)) +
              geom_col(aes(y = prop, fill = signature), alpha=0.5,  position = "stack") +
              geom_point(
                data = dnds_df,
                aes(x = start, y = (dnds - dnds_min) * scale_factor + min(signature_df$prop)),
              ...
              scale_y_continuous(
                sec.axis = sec_axis(
                  ~ (. - min(signature_df$prop)) / scale_factor + dnds_min)) +
              labs(x = "Month (since 2019/12)", fill = "Mutation signature", title="Nonsynonymous spectra") +
              my_theme() +
              theme(
                axis.title.x = element_blank(),
...
combined_plot <- plot_nonsyn / plot_syn +
                  my_theme()+
                  ...
                  guides(fill = guide_legend(nrow = 1),
                         title.position = "left")
ggsave("Figure4C.pdf",unit="cm", width=8, height=6,dpi = 300)
...
###Figure4F
#dn/ds and signature composition of H3N2
signature_df <- read_csv("../data/h3n2_signature_prop.csv")
signatrue_color <- c("#E64B35FF","#3C5488FF","#FFA500")
...
dnds_range <- dnds_max - dnds_min
scale_factor <- prop_range / dnds_range
dnds_signature_df <- signature_df %>% dplyr::left_join(dnds_df, by=c("year"="start"))

plot_syn <- ggplot(signature_df %>% filter(sub_type == "syn"), aes(x = year)) +
            geom_col(aes(y = prop, fill = signature), alpha=0.5,  position = "stack") +
            geom_point(
              data = dnds_df,
              aes(x = start, y = (dnds - dnds_min) * scale_factor + min(signature_df$prop)),
            ...
            scale_y_continuous(
              ...
              sec.axis = sec_axis(
                ~ (. - min(signature_df$prop)) / scale_factor + dnds_min,
                name = "dN/dS")) +
            labs(x = "Year (since 2000)", fill = "Mutation signature", title="Synonymous spectra") +
            my_theme() +
            theme(
              axis.title.y.right = element_text(angle = 90,face="italic"), 
              legend.position = "none")

plot_nonsyn <- ggplot(signature_df %>% filter(sub_type == "non_syn"), aes(x = year)) +
              geom_col(aes(y = prop, fill = signature), alpha=0.5,  position = "stack") +
              geom_point(
                data = dnds_df,
                aes(x = start, y = (dnds - dnds_min) * scale_factor + min(signature_df$prop)),
              ...
              scale_y_continuous(
                sec.axis = sec_axis(
                  ~ (. - min(signature_df$prop)) / scale_factor + dnds_min)) +
              labs(x = "Year (since 2000)", fill = "Mutation signature", title="Nonsynonymous spectra") +
              my_theme() +
              theme(
                axis.title.x = element_blank(),
                axis.text.x = element_blank(),
...
ggsave("Figure4F.pdf",unit="cm", width=8, height=6,dpi = 300)
